...
export class ProfileSettingsComponent implements OnInit {
  ...
  public ngOnInit() {
    ...
    this.projectSub = this.db.collection('broker', ref => ref.where('uid', '==', this.uid)).snapshotChanges().subscribe(async r => {
      const result = r[0].payload.doc.data();
  ...
  public async save() {
    if (this.viewClientForm.valid) {
      ...
      if (this.profpicfile) {
        ...
        console.log(this);
         const fileprop = await this.fileservice.upload_in_storage_percent(path, this.profpicfile, this.uid, 'client', this);
      ...
      this.brokerService.updateBroker(this.clientid, this.viewClientForm.value, x );
